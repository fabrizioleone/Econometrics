...
Kbeta         <- 2+dim(A)[2]                       # # of parameters in mean utility
Ktheta        <- 1+dim(A)[2]                       # # of parameters with random coefficient 
...
X             <- cbind(matrix(1, Total, 1), A, price)
Z             <- cbind(matrix(1, Total, 1), A, z, A^2, z^2)     # Instruments
nZ            <- dim(Z)[2]                         # # instrumental variables
W             <- solve(t(Z)%*%as.matrix(Z))                         # Starting GMM weighting matrix
true_vals     <- c(3, 3, 0.5, 0.5, -2, 0.8, 0.5, 0.5, 0.5)
